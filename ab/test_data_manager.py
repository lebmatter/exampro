...
class TestDataManager:
    ...
    def _create_exam_schedules(self):
        ...
        # Get predefined schedule names from data.json
        schedule_names = self.config["Exam Schedule"]["data"]
        
        for i, exam_name in enumerate(self.created_records["exams"]):
            # Create schedule starting 1 minute from now
            start_time = datetime.now() + timedelta(minutes=1)
            end_time = start_time + timedelta(hours=2)  # 2-hour window
            
            # Use predefined schedule name if available, otherwise generate one
            if i < len(schedule_names):
                predefined_schedule_name = schedule_names[i]
            else:
                predefined_schedule_name = f"Schedule-{exam_name}-{self.test_session_id[:8]}"
            
            schedule_name = self.client.create_exam_schedule(
                exam=exam_name,
                start_time=start_time.isoformat(),
                schedule_name=predefined_schedule_name,
                end_time=end_time.isoformat()
            )
            
            if schedule_name:
                self.created_records["schedules"].append(schedule_name)
                print(f"  ✅ Created schedule: {schedule_name} for exam: {exam_name}")
